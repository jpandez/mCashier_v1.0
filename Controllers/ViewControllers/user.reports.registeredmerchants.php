	...
	class SubscriberTransactionsController extends ViewController{
		public function __construct(){
			...
			if(isset($_SESSION["currentUser"]) && !empty($_SESSION["currentUser"]) && $this->verifyIP() && $this->getRolesConfig('REGISTERED_MERCHANTS_REPORT')){
				...
				if(isset($_REQUEST["Method"])){
					switch($_REQUEST["Method"]){
						case "RegisteredMerchants":
							//var_dump($_REQUEST["selecttype"]);
							$ret = $serv->registeredMerchantsReport($_REQUEST["selecttype"],$_REQUEST["perpage"],$_REQUEST["pagenum"]);
						...
						case "ExportRegisteredMerchantsCSV":
							//if($_REQUEST["queryCount"] <= 10000){
								...
								if(isset($ret)){
									...
									header("Content-Disposition: attachment; filename=registered_merchants_" .date("YmdHis") . ".csv");
									echo $out;
									die();
									exit;
								}
						...
						case "ExportRegisteredMerchantsEXCEL":
							//if($_REQUEST["queryCount"] <= 1500){
								...
								if(isset($ret)){								
									...
									header("Content-Disposition: attachment; filename=registered_merchants_" . date("YmdHis") . ".xls");
					...
					$ret = $serv->registeredMerchantsReport($_REQUEST["selecttype"],'15','1');
					$_SESSION['datatype'] = $_REQUEST['type'];
					$this->setData("registeredmerchantsdata",$ret);
					if(isset($ret->Token)){
						$_SESSION["token"] = $ret->Token;
				...
				$this->setMaster("user.reports.registeredmerchants.view.php");
				//$this->render();
			}else{
