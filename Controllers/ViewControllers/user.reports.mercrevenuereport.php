	...
	class SubscriberTransactionsController extends ViewController{
		public function __construct(){
			...
			if(isset($_SESSION["currentUser"]) && !empty($_SESSION["currentUser"]) && $this->verifyIP() && $this->getRolesConfig('TRANSACTION_REPORTS')){
				...
				if(isset($_REQUEST["Method"])){						
					switch($_REQUEST["Method"]){
						case "TransactionReports":
							if($_REQUEST["TransRdatefrom"] != '' && $_REQUEST["TransRdateto"] != '' && $_REQUEST["typeValue"] != ''){
								$datediff = $this->checkDateDiff($_REQUEST["TransRdatefrom"],$_REQUEST["TransRdateto"]);
								if($_SESSION['searchrange'] > $datediff){
									$ret = $serv->transactionReportsMercRevenue($_REQUEST["reportselecttype"],$_REQUEST["selecttype"],$_REQUEST["typeValue"],$_REQUEST["type"], $_REQUEST["TransRdatefrom"],$_REQUEST["TransRdateto"],$_REQUEST["perpage"],$_REQUEST["pagenum"]);
									$_SESSION['data'] = $ret;
						...
						case "ExportTransactionReportsCSV":
							//if($_REQUEST["queryCount"] <= 10000){
								$ret = $serv->transactionReportsMercRevenue($_REQUEST["reportselecttype"],$_REQUEST["selecttype"],$_REQUEST["typeValue"],$_REQUEST["type"], $_REQUEST["TransRdatefrom"],$_REQUEST["TransRdateto"],$_REQUEST["perpage"],$_REQUEST["pagenum"], 'all');
								if(isset($ret)){
									$out = $this->TRexportCSVmercrevenue($ret);
									header("Content-type: text/x-csv");								
									header("Content-Disposition: attachment; filename=transaction_mercrevenuereport_" .date("YmdHis") . ".csv");
									echo $out;
									die();
									exit;
								}
						...
						case "ExportTransactionReportsEXCEL":
							//if($_REQUEST["queryCount"] <= 1500){
								...
								if(isset($ret)){
									$out = $this->TRexportEXCELmercrevenue($ret);
									ob_end_clean();
									header("Content-type: application/octet-stream");
									header("Content-Disposition: attachment; filename=transaction_mercrevenuereport_" . date("YmdHis") . ".xls");
				...
				$this->setMaster("user.reports.mercrevenuereport.view.php");
				//$this->render();
			}else{
