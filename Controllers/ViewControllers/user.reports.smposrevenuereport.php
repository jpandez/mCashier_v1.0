			...
			if(isset($_SESSION["currentUser"]) && !empty($_SESSION["currentUser"]) && $this->verifyIP() && $this->getRolesConfig('TRANSACTION_HISTORY')){
				...
				if(isset($_REQUEST["Method"])){
					switch($_REQUEST["Method"]){
						case "SMposRevenueReport":
							if($_REQUEST["TransHdatefrom"] != '' && $_REQUEST["TransHdateto"] != ''){
								...
								if($_SESSION['searchrange'] > $datediff){
									...
									$ret = $serv->summaryMposRevenueReport($_REQUEST["TransHdatefrom"],$_REQUEST["TransHdateto"],$_REQUEST["perpage"],$_REQUEST["pagenum"]);
						...
						case "ExportSMposRevenueReportCSV":
							if($_REQUEST["queryCount"] <= 10000){
								$ret = $serv->summaryMposRevenueReport($_REQUEST["TransHdatefrom"],$_REQUEST["TransHdateto"],'10','1','all');
								
								if(isset($ret)){
									$out = $this->THexportCSV($ret);
									header("Content-type: text/x-csv");
									header("Content-Disposition: attachment; filename=summaryMPOSrevenueReport_" .date("YmdHis") . ".csv");
									echo $out;
									die();
									exit;
								}
						...
						case "ExportSMposRevenueReportEXCEL":
							if($_REQUEST["queryCount"] <= 1500){
								...
								if(isset($ret)){
									...
									$out = $this->SMRexportEXCEL($ret);
									ob_end_clean();
									header("Content-type: application/octet-stream");
									header("Content-Disposition: attachment; filename=summaryMPOSrevenueReport_" . date("YmdHis") . ".xls");
						...
						case "ExportSMposRevenueReportPDF":
							if($_REQUEST["queryCount"] <= 1500){
								$ret = $serv->summaryMposRevenueReport($_REQUEST["TransHdatefrom"],$_REQUEST["TransHdateto"],'10','1','all');
				...
				$this->setMaster("user.reports.smposrevenuereport.view.php");
				//$this->render();
			}else{
