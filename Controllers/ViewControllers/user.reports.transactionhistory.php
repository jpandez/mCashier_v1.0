	...
	class SubscriberTransactionsController extends ViewController{
		public function __construct(){
			...
			if(isset($_SESSION["currentUser"]) && !empty($_SESSION["currentUser"]) && $this->verifyIP() && $this->getRolesConfig('TRANSACTION_HISTORY')){
					...
					switch($_REQUEST["Method"]){
						case "TransactionHistory":
							if($_REQUEST["TransHdatefrom"] != '' && $_REQUEST["TransHdateto"] != ''){
								...
								if($_SESSION['searchrange'] > $datediff){
									...
									$ret = $serv->transactionHistoryDetailsSearchMPOS($_REQUEST["type"], $_REQUEST["TransHdatefrom"],$_REQUEST["TransHdateto"],$_REQUEST["perpage"],$_REQUEST["pagenum"]);
						...
						case "ExportTransactionHistoryCSV":
							//if($_REQUEST["queryCount"] <= 10000){
								$ret = $serv->transactionHistoryDetailsSearchMPOS($_REQUEST["type"], $_REQUEST["TransHdatefrom"],$_REQUEST["TransHdateto"],'10','1','all');
						...
						case "ExportTransactionHistoryEXCEL":
				...
				$this->setMaster("user.reports.transactionhistory.view.php");
				//$this->render();
			}else{
