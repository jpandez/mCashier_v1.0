	...
	class SubscriberTransactionsController extends ViewController{
		public function __construct(){
			...
			if(isset($_SESSION["currentUser"]) && !empty($_SESSION["currentUser"]) && $this->verifyIP() && $this->getRolesConfig('REGISTERED_MERCHANTS_REPORT')){
				...
				if(isset($_REQUEST["Method"])){
					switch($_REQUEST["Method"]){
						case "RegisteredMerchants":
							...
							$ret = $serv->registeredMerchantsInActiveReport($_REQUEST["selecttype"],$_REQUEST["perpage"],$_REQUEST["pagenum"],'page','90');
						...
						case "ExportRegisteredMerchantsCSV":
							//if($_REQUEST["queryCount"] <= 10000){
								$ret = $serv->registeredMerchantsInActiveReport($_REQUEST["selecttype"],'10','1','all','90');
								
								if(isset($ret)){
									$out = $this->RMexportCSVmpos($ret);
									header("Content-type: text/x-csv");
									header("Content-Disposition: attachment; filename=inactive_merchants_90days_active_" .date("YmdHis") . ".csv");
									echo $out;
									die();
									exit;
								}
						...
						case "ExportRegisteredMerchantsEXCEL":
							//if($_REQUEST["queryCount"] <= 1500){
								...
								if(isset($ret)){								
									...
									header("Content-Disposition: attachment; filename=inactive_merchants_90days_active_" . date("YmdHis") . ".xls");
						...
						case "ExportRegisteredMerchantsPDF":
							//if($_REQUEST["queryCount"] <= 1500){
								$ret = $serv->registeredMerchantsReport($_REQUEST["selecttype"],'10','1','all','90');
					...
					$ret = $serv->registeredMerchantsInActiveReport($_REQUEST["selecttype"],'15','1','page','90');
					$_SESSION['datatype'] = $_REQUEST['type'];
					$this->setData("registeredmerchantsdata",$ret);
					if(isset($ret->Token)){
						$_SESSION["token"] = $ret->Token;
				...
				$this->setMaster("user.reports.inactivemerchants90.view.php");
				//$this->render();
			}else{
